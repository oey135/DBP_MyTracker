...
function mapServerError(message) {
    if (!message)
        return { field: null, text: "알 수 없는 오류가 발생했습니다" };
    ...
    if (message === "duplicate_email")
        return {
            field: "email",
            text: "이미 가입된 이메일입니다. 로그인하거나 다른 이메일을 사용해주세요",
        };

    if (message === "duplicate_unknown")
        return {
            field: null,
            text: "이미 사용 중인 사용자명 또는 이메일입니다",
        };

    if (message.includes("이메일 또는 비밀번호"))
        return {
            field: "both",
            text: "이메일 또는 비밀번호가 틀렸습니다. 다시 확인해주세요",
        };
...
function validate(mode, form) {
    ...
    if (mode === "register") {
        ...
        if (!form.email) return { field: "email", text: "이메일을 입력하세요" };
        if (!form.password)
        ...
        >
            ...
            >
                ...
                <div>
                    ...
                    />
                    <FieldError
                        text={errors.email?.trim() ? errors.email : ""}
                    />
                </div>
                ...
                <div>
                    ...
                    />
                    <FieldError
                        text={errors.password?.trim() ? errors.password : ""}
                    />
                </div>
